<?php
    ...
    if($_POST['in-inputPDFFrom'] == "" || $_POST['in-inputPDFTo'] == ""){
        ...
        $stmt = $conn->prepare($sales_qry);
        $stmt->bind_param('ss',$_POST['in-inputPDFFrom'], $_POST['in-inputPDFTo']);
        $stmt->execute();
        $result = $stmt->get_result();
    }
    ...
    header('Content-Disposition: attachment; filename="my-pos-in-inventory-report-'.time().'.pdf"');
    echo $dompdf->output(); 
