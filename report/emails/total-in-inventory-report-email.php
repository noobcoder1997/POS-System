<?php
    ...
    try {
        ...
        if(mysqli_num_rows($result) > 0){
            ...
            if($_POST['in-inputEMAILTo'] == "" || $_POST['in-inputEMAILFrom'] == ""){

                $sales_qry = "SELECT *, SUM(quantity) AS qty FROM transactions WHERE status = '' AND type = 0 GROUP BY product_id";
                ...
                $sales_qry = "SELECT *, SUM(qty) AS qty FROM transactions WHERE (MONTH(date) BETWEEN ? AND ?) AND status = '' AND type = 0 GROUP BY product_id";
            
                $stmt = $conn->prepare($sales_qry);
                $stmt->bind_param('ss',$_POST['in-inputEMAILrom'], $_POST['in-inputEMAILTo']);
                $stmt->execute();
                $result = $stmt->get_result();
            }
            ...
            $mail->addAttachment($tempPdfPath, 'my-pos-in-inventory-report-'.time().'.pdf');
            
            //Content
            $mail->Subject = 'My POS INS Inventory';
            $mail->Body    = "Hi $email,<br><br>
                                We have received your request for the PDF file of the incoming inventory report. The report contains detailed information about the incoming transactions, including product details, quantities, statuses, and other relevant data. Please find the attached report in this email. If you have any questions or need further assistance, feel free to reach out to us. Thank you for using our service.
                                <br><br>
                                Best,<br>
                                My-POS<br>
