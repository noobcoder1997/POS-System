<?php
    ...
    try {
        ...
        if(mysqli_num_rows($result) > 0){
            ...
            $from = validate($_POST['in-inputEMAILTo']);
            $to = validate($_POST['in-inputEMAILFrom']);
            ...
            if( $from == "" || $to == ""){
                ...
                $sales_qry = "SELECT o.*, t.*, SUM(product_qty) AS qty FROM transactions t, order_items o WHERE o.product_qty = t.quantity AND o.product_id = t.product_id AND t.status = 0 AND t.type = 1 AND MONTH(date) BETWEEN ? AND ? GROUP BY t.product_id, t.date";
                $stmt = $conn->prepare($sales_qry);
            ...
            $mail->addAttachment($tempPdfPath, 'my-pos-total-sales-report-'.time().'.pdf');
            
            //Content
            $mail->Subject = 'My POS Total Sales Report';
            $mail->Body    = "Hi $email,<br><br>
                                We have received your request for the PDF file of the total sales report. The report contains detailed information about the sales transactions, including product details, quantities sold, statuses, and other relevant data. Please find the attached report in this email. If you have any questions or need further assistance, feel free to reach out to us. Thank you for using our service.
                                <br><br>
                                Best,<br>
                                My-POS<br>
