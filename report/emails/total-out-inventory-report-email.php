<?php
    ...
    try {
        ...
        if(mysqli_num_rows($result) > 0){
            ...
            if($_POST['out-inputEMAILTo'] == "" || $_POST['out-inputEMAILFrom'] == ""){
                ...
                $sales_qry = "SELECT *, SUM(qty) AS qty FROM transactions WHERE (MONTH(date) BETWEEN ? AND ?) AND status <> '' AND type = 1 GROUP BY product_id";
            
                $stmt = $conn->prepare($sales_qry);
                $stmt->bind_param('ss',$_POST['out-inputEMAILrom'], $_POST['out-inputEMAILTo']);
                $stmt->execute();
                $result = $stmt->get_result();
            }
            ...
            $mail->addAttachment($tempPdfPath, 'my-pos-out-inventory-report-'.time().'.pdf');
            
            //Content
            $mail->Subject = 'My POS OUTS Inventory';
