<?php
    ...
    if($_POST['in-inputXLSXFrom'] == "" || $_POST['in-inputXLSXTo'] == ""){
        ...
        $sales_qry = "SELECT *, SUM(quantity) AS qty FROM transactions WHERE (MONTH(date) BETWEEN ? AND ?) AND status = '' AND type = 0 GROUP BY product_id";
    
        $stmt = $conn->prepare($sales_qry);
        $stmt->bind_param('ss',$_POST['in-inputXLSXFrom'], $_POST['in-inputXLSXTo']);
        $stmt->execute();
        $result = $stmt->get_result();
    }
    ...
    header('Content-Disposition: attachment;filename="my-pos-in-inventory-report-'.time().'.xlsx"');
