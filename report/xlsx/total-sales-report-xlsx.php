<?php
    ...
    $sql = "SELECT o.*, t.*, SUM(product_qty) AS qty FROM transactions t, order_items o WHERE o.product_qty = t.quantity AND o.product_id = t.product_id AND t.status = 0 AND t.type = 1 GROUP BY t.product_id, t.date";
    ...
    while ($row = $result->fetch_assoc()) {
        // echo $row['product_id']."-".$row['qty'].$row['type']."\n";
        ...
        $sheet->setCellValue("D" . $rowIndex, number_format($row['product_price'],2)); // Set price in column D
    ...
    header('Content-Disposition: attachment;filename="my-pos-sales-report-'.time().'.xlsx"');
